...
class GameIdleMPVPlayer(QWidget):
    ...
    def __init__(self, file_name):
        ...
        if PLATFORM == "Intel":
            print("IDLE Intel")
            self.master_animated_image_player = mpv.MPV(wid=str(int(self.winId())), hwdec=config["hwdec"], vo=config["vo"], gpu_context="x11", loglevel="info", log_handler=self.mpv_log_handler)
        elif PLATFORM == "AMD":
            print("IDLE AMD")
            self.master_animated_image_player = mpv.MPV(wid=str(int(self.winId())), hwdec=config["hwdec"], vo=config["vo"], gpu_context="x11", loglevel="info", log_handler=self.mpv_log_handler)
        else:
