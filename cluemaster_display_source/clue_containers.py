...
class ClueVideoWidget(QWidget):
    ...
    def __init__(self, file_name):
        ...
        if PLATFORM == "Intel":
            print("Intel CLUE")
            self.master_clue_video_player = mpv.MPV(wid=str(int(self.winId())), hwdec="vaapi-copy", vo=config["vo"], gpu_context="x11egl", loglevel="info", log_handler=self.mpv_log_handler)
        elif PLATFORM == "AMD":
            print("AMD CLUE")
            self.master_clue_video_player = mpv.MPV(wid=str(int(self.winId())), hwdec="vaapi-copy", vo=config["vo"], gpu_context="x11egl", loglevel="info", log_handler=self.mpv_log_handler)
        else:
