...
class ClueVideoWidget(QWidget):
    ...
    def __init__(self, file_name):
        ...
        # widget
        if PLATFORM == "Intel":
            self.master_clue_video_player = mpv.MPV(wid=str(int(self.winId())), hwdec=config["hwdec"], vo=config["vo"], gpu_context=config["gpu_context"])
        elif PLATFORM == "AMD":
            self.master_clue_video_player = mpv.MPV(wid=str(int(self.winId())), hwdec=config["hwdec"], vo=config["vo"], gpu_context=config["gpu_context"])
        else:
...
class ClueAnimatedImageContainer(QWidget):
    ...
    def __init__(self, file_name):
        ...
        # widget
        if PLATFORM == "Intel":
            self.master_animated_image_player = mpv.MPV(wid=str(int(self.winId())), hwdec=config["hwdec"], vo=config["vo"], gpu_context=config["gpu_context"])
        elif PLATFORM == "AMD":
            self.master_animated_image_player = mpv.MPV(wid=str(int(self.winId())), hwdec=config["hwdec"], vo=config["vo"], gpu_context=config["gpu_context"])
        else:
...
class ClueWindow(QWidget):
    ...
    def master_image_clue_container(self, file_name):
        ...
        if default.endswith(".gif") or default.endswith(".apng") or default.endswith(".ajpg") or default.endswith(".jpg") or default.endswith(".png") or default.endswith(".jpeg"):
    ...
    def hide_clue_containers(self):
        ...
        if self.is_master_video_clue_playing is True:
            ...
            self.external_clue_video_window.setParent(None)
            self.unmute_game.emit(False)
            self.is_master_video_clue_playing = False
            ...
            self.external_clue_gif_window.setParent(None)
            self.is_animated_image_clue_playing = False

        elif self.is_svg_clue_visible is True:
            self.external_clue_animated_svg_renderer.close()
            self.external_clue_animated_svg_renderer.setParent(None)
            self.is_svg_clue_visible = False
