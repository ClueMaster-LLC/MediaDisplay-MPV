...
from cluemaster_display import authentication_screen, clue_containers, game_idle, loading_screen, master_overlay
import threads
import requests
...
class EndMediaWidget(QWidget):
    ...
    def __init__(self, file_name):
        ...
        # widgets
        if PLATFORM == "Intel":
            self.end_media_player = mpv.MPV(wid=str(int(self.winId())), hwdec=config["hwdec"], vo=config["vo"], gpu_context=config["gpu_context"])
        elif PLATFORM == "AMD":
            self.end_media_player = mpv.MPV(wid=str(int(self.winId())), hwdec=config["hwdec"], vo=config["vo"], gpu_context=config["gpu_context"])
        else:
...
class IntroVideoWindow(QWidget):
    ...
    def __init__(self):
        ...
        # widgets
        if PLATFORM == "Intel":
            self.master_intro_video_player = mpv.MPV(wid=str(int(self.winId())), hwdec=config["hwdec"], vo=config["vo"], gpu_context=config["gpu_context"])
        elif PLATFORM == "AMD":
            self.master_intro_video_player = mpv.MPV(wid=str(int(self.winId())), hwdec=config["hwdec"], vo=config["vo"], gpu_context=config["gpu_context"])
        else:
...
class NormalWindow(QMainWindow):
    ...
    def __init__(self):
        ...
        if PLATFORM == "Intel":
            self.master_video_player = mpv.MPV(wid=str(int(self.winId())), hwdec=config["hwdec"], vo=config["vo"], gpu_context=config["gpu_context"])
            self.master_picture_displayer = mpv.MPV(wid=str(int(self.winId())), hwdec=config["hwdec"], vo=config["vo"], gpu_context=config["gpu_context"])
    ...
    def download_files(self):
        ...
        if self.external_game_idle_window.mpv_player_triggered:
            self.external_game_idle_window.external_master_mpv_players.master_animated_image_player.quit()
        ...
        if game_status == 1:
            ...
            if self.is_game_idle is False:
                ...
                if self.is_intro_video_playing is True:
                    ...
                    self.external_intro_video_window.master_intro_video_player.quit()
                    ...
                    try:
                        ...
                        self.custom_end_media_widget.end_media_player.quit()
                    ...
                    try:
                        ...
                        self.custom_end_audio_media_widget.master_audio_player.quit()
    ...
    def master_intro_video_container(self):
        ...
        try:
            ...
            if get_intro_api_response != "No record found":
                ...
                self.external_intro_video_window = IntroVideoWindow()
