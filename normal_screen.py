...
class EndAudioMediaWidget(QWidget):
...
class IntroVideoWindow(QMainWindow):
    ...
    def __init__(self):
        ...
        # widgets
        self.master_intro_video_player = mpv.MPV(wid=str(int(self.winId())), hwdec="auto", vo="gpu")
...
class NormalWindow(QMainWindow):
    ...
    def __init__(self):
        ...
        self.master_video_player = mpv.MPV(wid=str(int(self.winId())), hwdec="auto", vo="gpu")
        self.master_picture_displayer = mpv.MPV(wid=str(int(self.winId())), hwdec="auto", vo="gpu")
    ...
    def master_end_media_container(self, status):
        ...
        if status == "won":
            if room_info_response["IsSuccessVideo"] is True:  # checking if the win video is enabled in the webapp
                default = "assets/room data/success end media/{}".format(os.listdir(os.path.join("assets/room data/success end media/"))[0])

                if default.endswith(".mp3"):
                    self.custom_end_audio_media_widget = EndAudioMediaWidget(file_name=default)
                    self.custom_end_audio_media_widget.hide()
                else:
            ...
            if room_info_response["IsFailVideo"] is True:  # checking if the lost video is enabled in the webapp
                default = "assets/room data/fail end media/{}".format(os.listdir(os.path.join("assets/room data/fail end media"))[0])
